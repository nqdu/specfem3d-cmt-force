submodule (static_module) static_impl
implicit none
...
12313 format(2(a,1x),15(g0,1x))
  ...
  subroutine static_problem_impl()
    ...
    use specfem_par, only: NGLOB_AB,NGLLX,NGLLY,NGLLZ,myrank,GPU_MODE,Mesh_pointer
    use specfem_par_elastic, only : displ
    implicit none
    ...
    ! GPU_MODE path (see run_cg_solver_gpu() in static_gpu.cu)
    real(kind=dp) :: rsfinal_gpu
    integer :: niter_gpu
    ...
    if(GPU_MODE) then
      if (ssol%cg_gpu_ptr == 0) call create_cg_gpu_backend()

      call run_cg_solver_gpu(ssol%cg_gpu_ptr, Mesh_pointer, NGLOB_AB, ssol%omega, &
                             rsinit, rsfinal_gpu, niter_gpu, &
                             displ, ssol%stress, ssol%strain)

      if (myrank == 0) then
        write(*,*)
        write(*,*) '----------------------------------------'
        write(*,*) 'GPU PCG Solver for Static Problem finished'
        write(*,*) 'Initial residual norm: ', rsinit
        write(*,*) 'Relative residual: ', rsfinal_gpu
        write(*,*) 'Iterations: ', niter_gpu
        write(*,*) '----------------------------------------'
      endif
  ...
  !> lazily creates the GPU CG solver backend (device arrays for the PCG vectors, external
  !! force, boundary condition data, stress/strain output), see prepare_cg_solver_gpu() in
  !! static_gpu.cu. Called once, from static_problem_impl() when GPU_MODE is enabled.
  subroutine create_cg_gpu_backend()
    use specfem_par, only: NGLOB_AB,NSPEC_AB,myrank,Mesh_pointer
    use specfem_par_elastic, only: nspec_inner_elastic,nspec_outer_elastic
    use specfem_par, only: num_fixed_bdry_faces,fixed_bdry_ispec,fixed_bdry_ijk
    use specfem_par, only: num_roller_bdry_faces,roller_bdry_ispec,roller_bdry_ijk,roller_bdry_normal
    use specfem_par, only: NPROC,nibool_interfaces_ext_mesh,my_neighbors_ext_mesh, &
                            buffer_send_vector_ext_mesh,buffer_recv_vector_ext_mesh, &
                            request_send_vector_ext_mesh,request_recv_vector_ext_mesh
    implicit none

    call prepare_cg_solver_gpu(ssol%cg_gpu_ptr, Mesh_pointer, NGLOB_AB, NSPEC_AB, myrank, &
                               nspec_outer_elastic, nspec_inner_elastic, &
                               ssol%force_ext, inv_mult_, &
                               num_fixed_bdry_faces, fixed_bdry_ispec, fixed_bdry_ijk, &
                               num_roller_bdry_faces, roller_bdry_ispec, roller_bdry_ijk, roller_bdry_normal, &
                               NPROC, &
